...
const CollapseView = () => {
    ...
    const handleOpenDrawer = (id: number) => {
        setRoomsData(
            oneProduct.deliveryInRooms.filter((item) => item.id === id)[0].data
        );
        setOpen(true);
    };
    ...
    return (
        <Container className={styles.container}>
            {oneProduct.deliveryInRooms.map((item: DataType, index: number) => (
                <div className="colapsibleMediaBox">
                    <Collapsible
                        ...
                        key={index}
                        onClose={() =>
                            setOpenId({ ...openId, [item.id]: false })
                        }
                    >
...
export default observer(CollapseView);
