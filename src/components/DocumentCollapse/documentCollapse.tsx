...
const DocumentCollapse: React.FC<Props> = ({ style }) => {
    const [active, setActive] = React.useState(1);

    const mainTrigger = (title: string, id: number) => (
        <div
            className={`${styles.triggerBox} ${
                active === id ? styles.triggerBoxTwo : ""
            }`}
        >
            <FileIcon />
            <p>{title}</p>
            <ArrowDownIcon
                className={active === id ? styles.iconActive : styles.icon}
            />
        </div>
    );
        ...
        <div className={styles.container} style={{ ...style }}>
            {data.map((item, index) => (
                <Collapsible
                    ...
                    trigger={mainTrigger(item.title, item.id)}
                    triggerWhenOpen={mainTrigger(item.title, item.id)}
                    open={active === item.id}
                    onOpening={() => setActive(item.id)}
                    onClosing={() => setActive(0)}
                >
